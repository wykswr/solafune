...
class MAEEncoder(nn.Module):
    def __init__(self, img_size=224, patch_size=16, num_frames=3, tubelet_size=1, 
                 in_chans=3, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4., 
                 norm_layer=nn.LayerNorm):
        super().__init__()

        # Initialize the encoder specifics
        self.patch_embed = PatchEmbed(img_size, patch_size, num_frames, tubelet_size, in_chans, embed_dim)
        num_patches = self.patch_embed.num_patches
    ...
    def initialize_weights(self):
        # Weights initialization remains the same
        pos_embed = get_3d_sincos_pos_embed(self.pos_embed.shape[-1], self.patch_embed.grid_size, cls_token=True)
        self.pos_embed.data.copy_(torch.from_numpy(pos_embed).float().unsqueeze(0))
    ...
    def forward(self, x):
        # Embedding patches
        x = self.patch_embed(x)
        x = x + self.pos_embed[:, 1:, :]  # Add position embedding without cls token

        # Append cls token and apply Transformer blocks
        cls_token = self.cls_token + self.pos_embed[:, :1, :]
        cls_tokens = cls_token.expand(x.shape[0], -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)
